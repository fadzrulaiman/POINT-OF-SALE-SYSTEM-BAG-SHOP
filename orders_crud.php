...
if (isset($_POST['create'])) {
    try {
        $stmt = $conn->prepare("INSERT INTO tbl_orders_a174485(fld_order_num, fld_staff_num,
...
if (isset($_POST['update'])) {
    if (isset($_SESSION['user']) && $_SESSION['user']['fld_staff_role'] == 'Admin') {
        try {
            $stmt = $conn->prepare("UPDATE tbl_orders_a174485 SET fld_staff_num = :sid,
...
if (isset($_GET['delete'])) {
    if (isset($_SESSION['user']) && $_SESSION['user']['fld_staff_role'] == 'Admin') {
        try {
            $stmt = $conn->prepare("DELETE FROM tbl_orders_a174485 WHERE fld_order_num = :oid");
            $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
...
if (isset($_GET['edit'])) {
    if (isset($_SESSION['user']) && $_SESSION['user']['fld_staff_role'] == 'Admin') {
        try {
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a174485 WHERE fld_order_num = :oid");
            $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
